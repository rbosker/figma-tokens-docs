...
## Bulk remap tokens
Click on `Bulk remap` to remap multiple tokens. A modal opens, fill in the token you want to remap in `Match` and put in the value where you want to remap it to in `Remap` input field. Afterwards click on the `Remap` button. 
This will replace the `Match` value and with the `Remap` value in all layers where the tokens were applied.

<img src='./bulk-remap.png' alt="bulk remap tokens" width="379" />
